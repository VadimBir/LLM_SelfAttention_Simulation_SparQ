...
def run_benchmark(excluded_steps):
    ...
    if 3 not in excluded_steps:
        ...
        # chmod 
        subprocess.run(["chmod", "+x", "./500-SOTA_PrefetcherSim_Runner_A14.sh"], check=True)
        ...
        subprocess.run(["./500-SOTA_PrefetcherSim_Runner_A14.sh"] + [base_Trace_Dir] + [base_Stats_Dir], check=True)
