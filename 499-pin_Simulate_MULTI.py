...
#base_Stats_Dir1 = "./000-simstats/comparisons"
S_LEN=4096 
# MUST DUPE THE CHAMPSIM DIR FOR A14 SIMULATION
base_Stats_Dir1 = f"750-tot_simStats_v4_MultiConf/A14"
base_Stats_Dir2 = f"750-tot_simStats_v4_MultiConf/glc"
...
def run_benchmark(excluded_steps):
    ...
    if 2 not in excluded_steps:
        ...
        try:
            for i, trace_path in enumerate(traceFileNames):
                ...
                subprocess.run(["xz", "-2", "--verbose", "-T8" ,trace_path], check=True)
    ...
    if 3 not in excluded_steps:
        ...
        # chmod 
        subprocess.run(["chmod", "+x", "./500-SOTA_PrefetcherSim_Runner_MultiHW_Config.sh"], check=True)

        # run another script to simulate the traces ./500-SOTA.sh <traces_base_path> <trace_file_log> 
        print ("giving args: ", base_Trace_Dir, base_Stats_Dir1, base_Stats_Dir2)
        subprocess.run(["./500-SOTA_PrefetcherSim_Runner_MultiHW_Config.sh"] + [base_Trace_Dir] + [base_Stats_Dir1] + [base_Stats_Dir2], check=True)
