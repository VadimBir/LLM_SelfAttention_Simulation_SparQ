...
# {IPDeltaMarkov:{NextDelta:count}}
 
COMBO=32
sed -i "s/^\(#define IP_TRACKER_COUNT \)[0-9]\+\(\.[0-9]*\)\{0,1\}/\1${COMBO}/" "$FILE_PATH"
sed -i "s/^\(#define PREFETCH_DEGREE \)[0-9]\+\(\.[0-9]*\)\{0,1\}/\1${COMBO}/" "$FILE_PATH"

# #define PREFETCH_DEGREE 20
sed -i "s/^\(const int MARKOV_LRU_SIZE = 1 + \)[0-9]\+\(\.[0-9]*\)\{0,1\};/\1${COMBO};/"  "$FILE_PATH"
#CHAMPSIM_DIR="/home/ubuntu/Directory/SparQ/pin_champsim"

COMBINATIONS=("0.0001" "50")
IP_STRIDE_PF_DEGREE_COMBINATIONS=("12" "64" "128") 
NESTED_COMBINATIONS=("2048" "32" "8" "64" "16")
...
total_runs=$((${#COMBINATIONS[@]}*${#NESTED_COMBINATIONS[@]}*${#TRACES_FILE[@]}*${#IP_STRIDE_PF_DEGREE_COMBINATIONS[@]}))

current_run=0
for COMBO in "${COMBINATIONS[@]}"; do
    for DegreeCombo in "${IP_STRIDE_PF_DEGREE_COMBINATIONS[@]}"; do
        for NestedCOMBO in "${NESTED_COMBINATIONS[@]}"; do
            ...
            for ((i=0; i<${#TRACES_FILE[@]}; i++)); do
                ...
                OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/022-TLayer1_|${output_type}|Phy70M_|L2-ip_stride_S|${NestedCOMBO}|_D|${DegreeCombo}|_|IPdelta-Markov-delta_S|${NestedCOMBO}|_Conf|${COMBO}_|L1-nextLine-1-2-4|.md"
                ...
                (
                    cd "$CHAMPSIM_DIR" && ./build_champsim.sh nxtln-1-2-4 030-L2-IPdeltaMarkovPercent_4k_ip_stride_L1-nextLine-1-2-4 no && cd ..
                    ./pin_champsim/bin/perceptron-nxtln-1-2-4-030-L2-IPdeltaMarkovPercent_4k_ip_stride_L1-nextLine-1-2-4-no-lru-1core -warmup 2000000 -simulation_instructions 350000000 -traces "$eachTraceFile" >> "$OUTPUT_FILE" 2>&1
    ...
    for DegreeCombo in "${IP_STRIDE_PF_DEGREE_COMBINATIONS[@]}"; do
        for NestedCOMBO in "${NESTED_COMBINATIONS[@]}"; do
            ...
            for ((i=0; i<${#TRACES_FILE[@]}; i++)); do
                ...
                OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/026-TLayer1_|${output_type}|Phy70M_|L2-nextLine-1-2-4|_ip_stride_S|${NestedCOMBO}|_D|${DegreeCombo}|_|L1-IPdelta-Markov-delta_S|${NestedCOMBO}|_Conf|${COMBO}|.md"
                #OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/022-TLayer1_${output_type}Phy70M_L2-ip_stride_S${NestedCOMBO}_D20_IPdelta-Markov-delta_S${NestedCOMBO}_Conf${COMBO}_L1-nextLine-1-2-4.md"
                echo "### Size: ${NestedCOMBO} CONFIDENCE: ${COMBO}" >> "$OUTPUT_FILE"
...
for NestedCOMBO in "${NESTED_COMBINATIONS[@]}"; do
    for ((i=0; i<${#TRACES_FILE[@]}; i++)); do
        ...
        OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/026-TLayer1_|${output_type}|Phy70M_|L2-IPCP_S|${NestedCOMBO}|_|L1-IPCP_S|${NestedCOMBO}|.md"
        #OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/022-TLayer1_${output_type}Phy70M_L2-ip_stride_S${NestedCOMBO}_D20_IPdelta-Markov-delta_S${NestedCOMBO}_Conf${COMBO}_L1-nextLine-1-2-4.md"
...
# Initial LLM Traces = 2B for 6 Transformer layers 
# ~~ 350M are the Layer 1, 
# All tests are done on only first layer traversal 
# simulation 
# Limit the number of processes to run concurrently
    ...
    for DegreeCombo in "${IP_STRIDE_PF_DEGREE_COMBINATIONS[@]}"; do
        for NestedCOMBO in "${NESTED_COMBINATIONS[@]}"; do
            ...
            for ((i=0; i<${#TRACES_FILE[@]}; i++)); do
                ...
                OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/026-TLayer1_|${output_type}|Phy70M_|L2-nextLine-1-2-4|_ip_stride_S|${NestedCOMBO}|_D|${DegreeCombo}|_|L1-Unfiltered-IPdelta-Markov-delta_S|${NestedCOMBO}|_Conf|${COMBO}|.md"
                #OUTPUT_FILE="/home/ubuntu/Directory/SparQ/005-ChampSim_Stats/LLM_Layer1_TOTALRUN_V3_DegreeTTT/022-TLayer1_${output_type}Phy70M_L2-ip_stride_S${NestedCOMBO}_D20_IPdelta-Markov-delta_S${NestedCOMBO}_Conf${COMBO}_L1-nextLine-1-2-4.md"
                echo "### Size: ${NestedCOMBO} CONFIDENCE: ${COMBO}" >> "$OUTPUT_FILE"
