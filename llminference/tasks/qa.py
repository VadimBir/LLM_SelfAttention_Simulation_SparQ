...
# evaluate LLM on TriviaQA
def evaluate_llm_on_triviaqa(adapter, data, batch_size=5, open_book=False):
    results = list(evaluate(adapter, [add_few_shot_prompt(data[i], 5, get_default_prompt_template(adapter.model.name_or_path, 5)) for i in range(10)], batch_size=batch_size, open_book=open_book))
    print(results[0])  # => {"id": ..., "output": ..., "match": True|False}
...
#         adapter,
#         [add_zero_shot_prompt(data[i]) for i in range(10)],
#         batch_size=5,
#         open_book=False,
#     ))
#     print(results[0])  # => {"id": ..., "output": ..., "match": True|False}
