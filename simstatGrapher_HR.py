...
# -------------------------------------------
# Plotting the Data (with AVG_L2_HR)
# -------------------------------------------

# Sort the DataFrame by AVG_L2_HR in descending order
df_sorted = df.sort_values(by='L2_HR', ascending=False).reset_index(drop=True)
df_sorted['Rank'] = df_sorted.index + 1  # Rank 1 is the highest AVG_L2_HR
...
# Extract unique table sizes and apply logarithmic normalization
...
# Create the scatter plot with AVG_L2_HR as the Y-axis
...
df_sorted = df_sorted[df_sorted['Max_Thresh'] <= 8]


scatter = plt.scatter(
    df_sorted['Rank'],
    df_sorted['L2_HR'],  # Y-axis is now L2_HR
    c=df_sorted['Color'],
...
for _, row in df_sorted.iterrows():
    ...
    plt.text(
        row['Rank'],
        row['L2_HR'] + 0.001,  # Slight offset to prevent overlapping with the dot
        label,
        fontsize=8,
        ha='center',
        va='bottom',
...
# Customize the plot
plt.title('Average L2_HR Ranked by Performance', fontsize=18)
plt.xlabel('Rank (1 = Highest AVG_L2_HR)', fontsize=14)
plt.ylabel('Average L2_HR', fontsize=14)  # Y-axis label changed to L2_HR
