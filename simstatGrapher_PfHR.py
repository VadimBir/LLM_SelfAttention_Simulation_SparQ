...
# -------------------------------------------
# Plotting the Data based on L2_Pf_HR
# -------------------------------------------

# Sort the DataFrame by L2_Pf_HR in descending order
df_sorted = df.sort_values(by='L2_Pf_HR', ascending=False).reset_index(drop=True)
df_sorted['Rank'] = df_sorted.index + 1  # Rank 1 is the highest L2_Pf_HR
...
scatter = plt.scatter(
    df_sorted['Rank'],
    df_sorted['L2_Pf_HR'],  # Change Y-axis to L2_Pf_HR
    c=df_sorted['Color'],
...
for _, row in df_sorted.iterrows():
    ...
    plt.text(
        row['Rank'],
        row['L2_Pf_HR'] + 0.001,  # Slight offset to prevent overlapping with the dot
        label,
        fontsize=8,
        ha='center',
        va='bottom',
...
# Customize the plot
plt.title('Average L2 Prefetch Hit Rate Ranked by Performance', fontsize=18)
plt.xlabel('Rank (1 = Highest L2_Pf_HR)', fontsize=14)
plt.ylabel('Average L2_Pf_HR', fontsize=14)
